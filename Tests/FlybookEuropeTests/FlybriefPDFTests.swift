...
final class FlybriefPDFTests: XCTestCase {
    func testFlybriefRendersOneSearchablePDFPagePerMainFlight() throws {
        ...
        XCTAssertTrue(fuelPage.string?.contains("START PLAN") == true)
        XCTAssertTrue(fuelPage.string?.contains("KRAFTSTOFFVERLAUF") == true)
        XCTAssertTrue(fuelPage.string?.contains("TANKSTOP") == true)
        XCTAssertTrue(fuelPage.string?.contains("STARTBESTAND") == true)
