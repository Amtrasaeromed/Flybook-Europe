...
struct DestinationMapWaypoint: Hashable {
    let latitude: Double
...
final class YearlyCachedTileOverlay: MKTileOverlay {
    ...
    override func loadTile(
        ...
        FlightNetwork.session.dataTask(with: remoteURL) {
...
struct DestinationMapView: NSViewRepresentable {
    ...
    var routeWaypoints: [DestinationMapWaypoint] = []
    ...
    func updateNSView(
        ...
        let locationKey =
            ...
            + routeWaypoints.map {
                "\($0.latitude),\($0.longitude),\($0.title),\($0.isDestination)"
            }.joined(separator: "|")
        ...
        if presentation == .europe {
            if !routeWaypoints.isEmpty {
                ...
                for (index, waypoint) in routeWaypoints.enumerated() {
                    ...
                    coordinates.append(waypointCoordinate)
                    let role: RouteAirportAnnotation.Role
                    if index == 0 {
                        role = .origin
                    ...
                    let annotation = RouteAirportAnnotation(role: role)
                    annotation.coordinate = waypointCoordinate
