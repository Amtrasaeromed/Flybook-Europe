...
final class RouteWindViewModel: ObservableObject {
    ...
    func load(
        ...
        do {
            ...
            // Best Level wird bewusst unabhaengig von den angezeigten
            // Dropdownwerten in 500-ft-Schritten ermittelt. Die Wetterprofile
            // werden dabei nur einmal geladen und anschliessend lokal je Hoehe
            // interpoliert; das Raster erzeugt keine zusaetzlichen Downloads.
            let optimizationLevels = Array(
                stride(from: 2_000, through: 10_000, by: 500)
            )
            ...
            let candidates = result.filter {
                (2_000...10_000).contains($0.altitudeFeet)
                    && $0.altitudeFeet.isMultiple(of: 500)
            }
